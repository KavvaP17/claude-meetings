...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path — do not assume a particular working directory. Replace `<skill-dir>` below with this skill's actual absolute path (announced when the skill is invoked; otherwise it's `.claude/skills/ui-ux-pro-max` under the project root):
